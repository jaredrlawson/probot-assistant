...
// (DROP-IN) Keeps split architecture; passes $ctx to the view
// ---------------------------------------------
if (!defined('ABSPATH')) exit;

// Ensure settings are registered even if view loads first.
require_once dirname(__FILE__) . '/admin-settings-register.php';

/**
 * Render Settings page — gathers options, passes compact $ctx to view.
 * View file: includes/views/settings-page.php
 */
function probot_render_settings_page() {
  if (!current_user_can('manage_options')) return;

  // Gather options (logic only; no markup here)
  $ctx = array(
    // Core
  ...
  // Expose individual vars for the view (compat with your current template)
  extract($ctx, EXTR_SKIP);

  // Render the pure markup template, keeping $ctx available for hooks
